...
```{r}
# Create a bar chart
bar_chart <- final_data %>%
  count(Location) %>%
  ggplot(aes(x = reorder(Location, n), y = n)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Incidents by Location (Bar Chart)",
       x = "Location",
       y = "Incident Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Assuming you have shapefiles or spatial data for boundaries of each location
...
clustered_bar_chart <- final_data %>%
  group_by(ClusterVariable) %>%
  count(Location) %>%
  ggplot(aes(x = reorder(Location, n), y = n, fill = ClusterVariable)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Incidents by Location (Clustered Bar Chart)",
       x = "Location",
       y = "Incident Count",
       fill = "Cluster") +
  theme_minimal() +
